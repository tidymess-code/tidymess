...
void Evolver_direct::leapfrog_step(vector<Body> &bodies, double dt, Force *force) {
    double dth = 0.5*dt;
...
void Evolver_direct::mclachlan_step(vector<Body> &bodies, double dt, Force *force) {   
...
void Evolver_direct::initialize(vector<Body> &bodies, Force *force) {
    shape.calculate_I_inv_and_w(bodies);
...
void Evolver_direct::evolve_model(double &t, double &t_end, vector<Body> &bodies, Force *force, Timestep_base *timestep) {
...
void Evolver_direct::evolve_model(double &t, int &N_step, vector<Body> &bodies, Force *force, Timestep_base *timestep) {
    num_integration_step = 0;
