...
void Evolver_direct_col_mb::leapfrog_step(vector<Body> &bodies, double dt, Force *force) {
    double dth = 0.5*dt;
    ...
    force->update_acceleration_and_torque_and_deformation_tensor(bodies, true);
    ...
    force->update_acceleration_and_torque_and_deformation_tensor(bodies, false);
    force->update_centrifugal_deformation(bodies);
    force->update_equilibrium_tensor(bodies);
...
void Evolver_direct_col_mb::leapfrog_step_with_collisions(vector<Body> &bodies, double dt, Force *force) {
    double dth = 0.5*dt;
    ...
    force->update_acceleration_and_torque_and_deformation_tensor_col(bodies, true);
    force->update_centrifugal_deformation(bodies);
    force->update_equilibrium_tensor(bodies);
...
void Evolver_direct_col_mb::mclachlan_step(vector<Body> &bodies, double dt, Force *force) {   
...
void Evolver_direct_col_mb::initialize(vector<Body> &bodies, Force *force) {
    shape.calculate_I_inv_and_w(bodies);
...
void Evolver_direct_col_mb::evolve_model(double &t, double &t_end, vector<Body> &bodies, Force *force, Timestep_base *timestep) {
...
void Evolver_direct_col_mb::evolve_model(double &t, int &N_step, vector<Body> &bodies, Force *force, Timestep_base *timestep) {
    num_integration_step = 0;
