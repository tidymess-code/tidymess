...
// Evolve function
void Evolver_nbody_pn::leapfrog_step(vector<Body> &bodies, double dt, Force *force) {
    double dth = 0.5*dt;
...
void Evolver_nbody_pn::mclachlan_step(vector<Body> &bodies, double dt, Force *force) {
...
void Evolver_nbody_pn::initialize(vector<Body> &bodies, Force *force) {
    force->update_acceleration_pn(bodies, false);
}
void Evolver_nbody_pn::evolve_model(double &t, double &t_end, vector<Body> &bodies, Force *force, Timestep_base *timestep) {
...
void Evolver_nbody_pn::evolve_model(double &t, int &N_step, vector<Body> &bodies, Force *force, Timestep_base *timestep) {
    num_integration_step = 0;
